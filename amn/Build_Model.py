...
def input_ANN_Dense(parameter, verbose=False):
    ...
    from amn.Utilities import MaxScaler
...
def model_input(parameter, trainable=True, verbose=False):
    from amn.Build_Reservoir import input_RC
...
def model_type(parameter, verbose=False):
    from amn.Build_Reservoir import RC
...
def train_model(parameter, Xtrain, Ytrain, Xtest, Ytest, 
    ...
    if verbose:
        if temperature:
            from amn.Build_Reservoir import identical_media
            ...
            found, size = 0, 0
        print(f'train = {otrain:.2f} test = {otest:.2f} loss-train = {ltrain:.6f} loss-test = {ltest:.6f}')
...
class Neural_Model:
    ...
    def __init__(self,
        ...
        self.niter = niter
        self.xfold = xfold
        self.cobraname_override = cobraname_override
    ...
    def get_parameter(self, init=False, objective=False, verbose=False):
        ...
        parameter.load(self.trainingfile, cobraname_override=self.cobraname_override)
        if verbose == 2:
        ...
        if verbose == 2:
            print('4. in get_parameter self.objective parameter.Y.shape self.output_dim', 
                  self.objective, parameter.Y.shape, self.output_dim)
        ...
        if self.output_dim != parameter.Y.shape[1]:
            ...
            self.output_dim = end
        if verbose == 2:
            print('5. in get_parameter self.objective self.Y.shape self.output_dim', 
    ...
    def load(self, filename, fileparam=None, filemodel=None, output_dim=-1, verbose=False):
        ...
        with open(fileparam, 'r') as h:
            for line in h:
                K = line.rstrip().split(',')
        # model architecture
        loaded_trainingfile = str(K[0])
        if self.trainingfile is None:
            self.trainingfile = loaded_trainingfile
    ...
    def printout(self, filename=''):
        ...
        if self.trainingfile:
            if os.path.isfile(f'{self.trainingfile}.npz'):
                ...
                print(f'training set medium {len(self.medium)}')
                print(f'training set S {self.S.shape}')
                print(f'training set Pin {self.Pin.shape}')
                print(f'training set Pko {self.Pko.shape}')    
                print(f'training set Pout {self.Pout.shape}')
        else:
