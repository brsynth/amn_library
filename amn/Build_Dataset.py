...
def create_random_medium_cobra(model, objective, 
    ...
    if obj < cobra_min_objective and verbose > 1:
        print(f'Cobra objective is below min {cobra_min_objective}') 
    if obj > cobra_max_objective and verbose > 1:
        print(f'Cobra objective is above max {cobra_max_objective}') 
...
def read_trainingfile(filename):
    ...
    # - medium
    # - X and Y values
    from amn.Utilities import read_csv
...
class TrainingSet:
    ...
    def load(self, filename, cobraname_override=None):
        ...
        self.X = loaded['X']
        self.Y = loaded['Y']
        xml_path = cobraname_override if cobraname_override else self.cobraname
        self.model = cobra.io.read_sbml_model(xml_path + ".xml")
    ...
    def get(self, sample_size=100, mediumexp=[], genekos=[], 
        ...
        while I < sample_size:
            ...
            if zeroflux or np.sum(y): # only non null solutions
                X[I], Y[I] = x, y
                I += 1
    ...
    def filter_measure(self, measure=[], verbose=False):
        ...
        if len(self.measure) > 0:
            ...
            print(self.Y.shape, self.Pout.shape)
